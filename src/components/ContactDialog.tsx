...
export default function ContactDialog({type, visible, onHide}: ContactDialogProps ) {
    const phoneContacts: PhoneContact[] = [
        { clinic: "Clinica Escola de Saúde e Imagem (CESIU", phone: "3306-8232 e 3306-8933" },
    ...
    const emailContacts: EmailContact[] = [
        { clinic: "Serviço Escola de Psicologia Aplicada (SEPA)", email: "secsepa@unichristus.edu.br" },
      ];
        ...
        >
            <div className="p-6">
                ...
                <div className="space-y-4">
                    {data.map((item, index) => (
                        <div key={index} className="flex flex-col p-4 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
                            <div>
                                <h3 className="font-semibold text-gray-800">{item.clinic}</h3>
                                <p className="text-gray-600 text-sm flex items-center gap-2">
                                    <span className="font-medium">{contactLabel}</span>
                                    <span>{isPhone ? (item as PhoneContact).phone : (item as EmailContact).email}</span>
                                </p>
                            </div>
                        </div>
                    ))}
