...
export default function Schedule() {
  ...
  useEffect(() => {
    const fetchSpecialties = async () => {
      try {
        ...
        const activeSpecialties = data.filter((specialty: Specialty) => !specialty.disabled);
        setSpecialties(activeSpecialties);
      } catch (err) {
        console.error("Erro ao carregar especialidades:", err);
        setError("Não foi possível carregar as especialidades. Por favor, tente novamente mais tarde.");
      } finally {
      ...
      if (exists) {
        ...
        try {
          ...
          if (birthDate && !birthDate.includes('/')) {
            ...
            if (parts.length === 3) {
              // Não converter para DD/MM/YYYY no state, pois o input date espera YYYY-MM-DD
  ...
  const handlePersonalInfoSubmit = async (e: { preventDefault: () => void }) => {
    ...
    if (!cadastroExistente) {
      try {
        ...
        // Se o cadastro foi bem-sucedido, mostra mensagem e avança
  ...
  const handleSubmit = (e: { preventDefault: () => void }) => {
    ...
    console.log("Dados antes do agendamento:", { cpf, nome, dataNas, telefone, especialidade });
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  ...
                  <div className="bg-gray-100 p-3 rounded-lg mb-4">
                    <h3 className="font-semibold text-gray-700 mb-2">Dados do Paciente:</h3>
                    <p><span className="font-medium">Nome:</span> {nome}</p>
                    <p><span className="font-medium">CPF:</span> {cpf}</p>
                    <p><span className="font-medium">Data de Nascimento:</span> {formatDateForDisplay(dataNas)}</p>
                    <p><span className="font-medium">Telefone:</span> {telefone}</p>
                  </div>
                  <label
                    className="block text-gray-600 mb-2 font-semibold"
                  ...
                  {loadingSpecialties ? (
                    <p className="text-gray-500 mb-4">Carregando especialidades...</p>
                  ) : (
                    <select
                      id="especialidade"
                      value={especialidade}
                    ...
                    >
                      ...
                      {specialties.map((specialty) => (
                        <option key={specialty.id} value={specialty.id}>
                          {specialty.description} ({specialty.specialtyType.description})
                        </option>
                      ))}
                    </select>
                  )}
                ...
                <button
                  ...
                  className={`${
                    loadingSpecialties ? "bg-gray-400" : "bg-green-500 hover:bg-green-600"
                  } text-white p-3 rounded-lg w-full transition duration-300`}
                >
